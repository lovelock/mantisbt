...
namespace Mantis\Core\Api;

use Core\Api\BugData;
...
# @global array $g_cache_access_matrix
    ...
    public static $g_cache_access_matrix = array();

# @global array $g_cache_access_matrix_project_ids
    public static $g_cache_access_matrix_project_ids = array();

# @global array $g_cache_access_matrix_user_ids
    public static $g_cache_access_matrix_user_ids = array();
        ...
        if (!auth_is_user_authenticated()) {
            ...
            if (current_user_is_anonymous()) {
                if (basename($_SERVER['SCRIPT_NAME']) != 'login_page.php') {
                    ...
                    print_button(helper_mantis_url('login_page.php') . '?return=' . $t_return_page,
                        lang_get('click_to_login'));
     ...
     * @access private
     */
    public static function access_cache_matrix_project($p_project_id)
    {
     ...
     * @access private
     */
    public static function access_cache_matrix_user($p_user_id)
    {
     ...
     * @access public
     */
    public static function access_compare_level($p_user_access_level, $p_threshold = NOBODY)
    {
     ...
     * @access public
     */
    public static function access_get_global_level($p_user_id = null)
    {
        if ($p_user_id === null) {
     ...
     * @access public
     */
    public static function access_has_global_level($p_access_level, $p_user_id = null)
    {
        ...
        $t_access_level = self::access_get_global_level($p_user_id);

        return self::access_compare_level($t_access_level, $p_access_level);
    }
     ...
     * @return void
     */
    public static function access_ensure_global_level($p_access_level, $p_user_id = null)
    {
        if (!self::access_has_global_level($p_access_level, $p_user_id)) {
            self::denied();
        }
    }
     ...
     * @access public
     */
    public static function access_get_project_level($p_project_id = null, $p_user_id = null)
    {
        if (null === $p_user_id) {
            $p_user_id = auth_get_current_user_id();
        }
        ...
        $t_global_access_level = self::access_get_global_level($p_user_id);

        if (ALL_PROJECTS == $p_project_id || user_is_administrator($p_user_id)) {
            return $t_global_access_level;
        } else {
            $t_project_access_level = self::access_get_local_level($p_user_id, $p_project_id);
            ...
            if (false === $t_project_access_level) {
                ...
                if (VS_PRIVATE == $t_project_view_state) {
                    if (self::access_compare_level($t_global_access_level,
                        config_get('private_project_threshold', null, null, ALL_PROJECTS))) {
                        return $t_global_access_level;
                    } else {
                        return ANYBODY;
                    }
     ...
     * @access public
     */
    public static function access_has_project_level($p_access_level, $p_project_id = null, $p_user_id = null)
    {
        # Short circuit the check in this case
        if (NOBODY == $p_access_level) {
            return false;
        ...
        $t_access_level = self::access_get_project_level($p_project_id, $p_user_id);
     ...
     * @return void
     */
    public static function access_ensure_project_level($p_access_level, $p_project_id = null, $p_user_id = null)
    {
        if (!self::access_has_project_level($p_access_level, $p_project_id, $p_user_id)) {
            self::denied();
        }
    }
     ...
     * @access public
     */
    public static function access_has_any_project($p_access_level, $p_user_id = null)
    {
        # Short circuit the check in this case
        if (NOBODY == $p_access_level) {
            return false;
        ...
        foreach ($t_projects as $t_project) {
            if (self::access_has_project_level($p_access_level, $t_project['id'], $p_user_id)) {
                return true;
            }
        }
     ...
     * @access public
     */
    public static function access_has_bug_level($p_access_level, $p_bug_id, $p_user_id = null)
    {
        if ($p_user_id === null) {
            $p_user_id = auth_get_current_user_id();
        }
        ...
        $t_access_level = self::access_get_project_level($t_project_id, $p_user_id);
        ...
        if ($t_limit_reporters && !$t_bug_is_user_reporter) {
            ...
            if (!self::access_compare_level($t_access_level, $s_thresholds[$t_project_id])) {
                return false;
            }
        }
        ...
        if (!$t_bug_is_user_reporter && bug_get_field($p_bug_id, 'view_state') == VS_PRIVATE) {
            ...
            return self::access_compare_level($t_access_level, $t_private_bug_threshold)
            && self::access_compare_level($t_access_level, $p_access_level);
        }
     ...
     */
    public static function access_has_bug_level_filter($p_access_level, $p_bug_id, $p_user_ids)
    {
        $t_users_ids_with_access = array();
        foreach ($p_user_ids as $t_user_id) {
            if (self::access_has_bug_level($p_access_level, $p_bug_id, $t_user_id)) {
     ...
     * @access public
     */
    public static function access_ensure_bug_level($p_access_level, $p_bug_id, $p_user_id = null)
    {
        if (!self::access_has_bug_level($p_access_level, $p_bug_id, $p_user_id)) {
            self::denied();
        }
    }
     ...
     * @access public
     */
    public static function access_has_bugnote_level($p_access_level, $p_bugnote_id, $p_user_id = null)
    {
        if (null === $p_user_id) {
            $p_user_id = auth_get_current_user_id();
        }
        ...
        if (bugnote_get_field($p_bugnote_id, 'view_state') == VS_PRIVATE && !bugnote_is_user_reporter($p_bugnote_id,
                $p_user_id)
        ) {
        ...
        return self::access_has_bug_level($p_access_level, $t_bug_id, $p_user_id);
    }
     ...
     * @return array filtered array of user ids.
     */
    public static function access_has_bugnote_level_filter($p_access_level, $p_bugnote_id, $p_user_ids)
    {
        $t_users_ids_with_access = array();
        foreach ($p_user_ids as $t_user_id) {
            if (self::access_has_bugnote_level($p_access_level, $p_bugnote_id, $t_user_id)) {
                $t_users_ids_with_access[] = $t_user_id;
            }
        }
     ...
     * @return void
     */
    public static function access_ensure_bugnote_level($p_access_level, $p_bugnote_id, $p_user_id = null)
    {
        if (!self::access_has_bugnote_level($p_access_level, $p_bugnote_id, $p_user_id)) {
            self::denied();
        }
    }
     ...
     * @access public
     */
    public static function access_can_close_bug(BugData $p_bug, $p_user_id = null)
    {
     ...
     * @return void
     */
    public static function access_ensure_can_close_bug(BugData $p_bug, $p_user_id = null)
    {
     ...
     * @access public
     */
    public static function access_can_reopen_bug(BugData $p_bug, $p_user_id = null)
    {
        ...
        if (ON == config_get('allow_reporter_reopen', null, null, $p_bug->project_id)
            && bug_is_user_reporter($p_bug->id, $p_user_id)
            && self::access_has_project_level(config_get('report_bug_threshold', null, $p_user_id, $p_bug->project_id),
                $p_bug->project_id, $p_user_id)
        ) {
            return true;
        }
        ...
        if (self::access_has_bug_level($t_reopen_bug_threshold, $p_bug->id, $p_user_id)) {

            # User must be allowed to change status to reopen status
            $t_reopen_status_threshold = self::access_get_status_threshold($t_reopen_status, $p_bug->project_id);
            return self::access_has_bug_level($t_reopen_status_threshold, $p_bug->id, $p_user_id);
        }
     ...
     * @return void
     */
    public static function access_ensure_can_reopen_bug(BugData $p_bug, $p_user_id = null)
    {
        if (!self::access_can_reopen_bug($p_bug, $p_user_id)) {
            self::denied();
        }
    }
     ...
     * @access public
     */
    public static function access_get_local_level($p_user_id, $p_project_id)
    {
        global $g_cache_access_matrix, $g_cache_access_matrix_project_ids;
        ...
        $t_project_level = self::access_cache_matrix_user($p_user_id);
     ...
     * @access public
     */
    public static function access_get_status_threshold($p_status, $p_project_id = ALL_PROJECTS)
    {
     ...
     * @return string
     */
    public static function access_level_get_string($p_access_level)
    {
